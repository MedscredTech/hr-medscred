...
def admin_dashboard():
    ...
    if selected_date_str:
        try:
            ...
            selected_date = get_ist_date()  # Use IST date instead of date.today()
    else:
        selected_date = get_ist_date()  # Use IST date instead of date.today()
    ...
    try:
        with conn.cursor() as cursor:
            ...
            # 🚨 NEW: Check if selected date is a holiday
            cursor.execute("""
                SELECT holiday_name, description
                FROM t_holidays_hr 
                WHERE holiday_date = %s AND is_active = 1
            """, (selected_date,))
            holiday_info = cursor.fetchone()
    ...
    return render_template(
        ...
        records=records,
        selected_date=selected_date,
        today_date=get_ist_date(),  # Use IST date instead of date.today()
        summary=summary,
        pending_checkouts=pending_checkouts,
        holiday_info=holiday_info  # 🚨 NEW: Add holiday information
    )
...
#             with conn.cursor() as cursor:
#                 if action == 'checkin':
#                     # 🚨 NEW LOGIC: Handle previous day incomplete checkout FIRST
#                     cursor.execute("""
#                         SELECT id, checkin_time, DATE(checkin_time) as checkin_date
#                         FROM t_attendance_hr 
#                         WHERE user_id = %s 
...
#                     if pending_checkout:
#                         previous_date = pending_checkout['checkin_date']
#                         # Set checkout time to 23:59:59 of previous day
...
#                                     check_out_location_name = CONCAT('Auto-checkout on next day check-in: ', 
#                                                                    COALESCE(check_in_location_name, 'System'))
#                                 WHERE id = %s
#                             """, (auto_checkout_time, auto_checkout_time, pending_checkout['id']))
#                         else:
#                             # Use old column structure for backward compatibility
#                             cursor.execute("""
#                                 UPDATE t_attendance_hr 
#                                 SET checkout_time = %s,
#                                     work_duration = TIMEDIFF(%s, checkin_time),
...
#                         print(f"✅ Auto-checkout completed for User ID: {user_id}, Date: {previous_date}")
...
#                              location_name)
#                         )
#                         checkout_msg = " Previous day automatically closed." if pending_checkout else ""
#                         msg = f"Checked in successfully at {get_ist_time_for_display()}.{checkout_msg}"
#                         msg_type = 'success'
...
#     # ✅ UPDATED RETURN STATEMENT WITH HOLIDAY INTEGRATION
#     return render_template(
#         'dashboard_user.html',
#         firstname=user['firstname'] if user else '',
#         lastname=user['lastname'] if user else '',
...
#         current_time=get_ist_now(),
#         current_year=get_ist_date().year,
#         upcoming_holidays=upcoming_holidays,  # 🚨 NEW: Add holidays
#         today_date=get_ist_date()             # 🚨 NEW: Add today's date for countdown
#     )
@app.route('/user_dashboard', methods=['GET', 'POST'])
def user_dashboard():
    # Authentication check
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            with conn.cursor() as cursor:
                if action == 'checkin':
                    # Handle previous day incomplete checkout FIRST
                    cursor.execute("""
                        SELECT id, checkin_time, DATE(checkin_time) as checkin_date
                        FROM t_attendance_hr 
                        WHERE user_id = %s 
        ...
        # 🚨 NEW: Get all holidays for dropdown (current year + next year)
        current_year = get_ist_date().year
        with conn.cursor() as cursor:
            cursor.execute("""
                SELECT holiday_date, holiday_name, description, DAYNAME(holiday_date) as day_name
                FROM t_holidays_hr 
                WHERE YEAR(holiday_date) IN (%s, %s) 
                AND is_active = 1
                ORDER BY holiday_date ASC
    ...
    return render_template(
        ...
        upcoming_holidays=upcoming_holidays,  # For holiday notifications/cards
        all_holidays=all_holidays,            # For dropdown list in template
...
@app.route('/holidays')
def holidays_list():
    if session.get('role') != 2:  # Admin only
        return redirect(url_for('login'))
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute("""
                SELECT id, holiday_date, holiday_name, description, is_active
                FROM t_holidays_hr 
...
def add_holiday():
    ...
    if request.method == 'POST':
        ...
        try:
            with conn.cursor() as cursor:
                cursor.execute("""
                    INSERT INTO t_holidays_hr (holiday_date, holiday_name, description)
                    VALUES (%s, %s, %s)
    ...
    return render_template('add_holiday.html',today_date=get_ist_date())
...
def delete_holiday(holiday_id):
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute("DELETE FROM t_holidays_hr WHERE id = %s", (holiday_id,))
...
def get_holiday_name(date):
    ...
    try:
        with conn.cursor() as cursor:
            ...
            """, (date,))
            result = cursor.fetchone()
            return result['holiday_name'] if result else None
    finally:
        conn.close()
